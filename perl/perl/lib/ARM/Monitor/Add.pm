...
#               [ @of_worksheets ]  Monitor :: Device specific information
...
use version; our $VERSION =version->declare("v1.01.14");
...
sub     read_specification  {
        ...
        #write_array_to_file(${$opts_ref}{tmp} ,'logs/parse_xlsx.log');             # Dead code
        #write_array_to_file(${$m}{svi} ,'axi_monitor_include.svi');                # Dead code
...
sub     signal_connecting   {
        ...
        my $axi4    = ($prot =~ m/AXI4/)?$TRUE:$FALSE;                  #   Set axi protocol
        ...
        foreach my $signal ( @{${$monitor}{$device}} ) {
            unless ( $signal =~ m/$device/ ) {                          #   Exclude the Header, not a signal apendix
                ...
                my $cmt = ($signal =~ m/\/\// )? $TRUE:$FALSE;          #   Commenting Signal port
