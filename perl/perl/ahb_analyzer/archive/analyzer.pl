...
# Phone     408 807 6915
#
# Purpose   Analysis AHB-tracker trace files
#
# Revision  VERSION     DATE            Description
# ================================================================================================
# History   1.01.01     10/22/2018      Create xxx analyzing script
#
...
my %opts    =   (  #from                =>   optional,          # Optional Command line options -- No default settings
                   ...
                   #debug               =>   optional,                                          -- No default settings
                    ...
                    coverage            =>  'coverage.rpt',     # Default logname for coverage report
...
GetOptions  (   'help|h|?'              =>  \&help,                         # Usage Information
                #'analyze|ana'          =>  \&addr_cov(,                     # --        testing address coverage
   ...
   #$data_obj{slave_index}    NA       13                       # Slave index
    ...
    if ( !defined ${${$ptr_ref}{adr}{previous}}{data} ) {
         ...
         if ( $data_obj{assert} == ${${$ptr_ref}{adr}{previous}}{assert} ) {
              #if ( !defined ${${${$ptr_ref}{adr}{previous}}{prev}}{data}  ) {
         ...
         #
              ...
              printf $buih  "%*s:: sequential  AHB data    transfer phase detected FIRST\n", 22, '';
              printf $buih " %*sDATA transfer assertion time : %8s ns\n", 24, '',$data_obj{assert};
              printf $buih " %*sADDR transfer assertion time : %8s ns\n", 24, '',${${$ptr_ref}{adr}{previous}}{assert};
...
sub   latency_analysis{
      ...
      my $frequency   =     ${$trace_header_ref}{frequency};                    # (float)
      my $fb          =     ${$trace_header_ref}{freqbase};                     # (MHz|GHz)
     #my $ptr_cur_cmd   =   ${$ptr_ref}{adr}{first};                            # This should be the unified pointer
     #my $ptr_cur_cmd =   ${${$ptr_ref}{start}}; 
      my $ptr_cur_cmd =     ${$ptr_ref}{start};                                 # See       restrict_window() there is no START for DATA transfers
     #my $buffer_stop =   ${${$ptr_ref}{stop}};                                 # Last transaction shall be included in window
     ...
     #my $ptr_cur_cmd =   ${${$ptr_ref}{first}};                                #       Deprecated for full trace iteration
     #my $buffer_stop =   ${${$ptr_ref}{first}};                                #       Deprecated for full trace iteration
...
sub   restrict_window {
     ...
     #printf " %s\n", '='x80;
      ...
      if ( !defined ${$opts_ref}{from}  ) {
            #${$opts_ref}{from} = ${${${$ptr_ref}{adr}{first}}}{assert};            # Set default old hard reference
     ...
     #printf " TraceStart: %10s %s\n", ${${$ptr_ref}{adr}{first}}{assert}, $tb;     # Develop control flow - Silenced on terminal
...
sub   transaction2_analysis{
      ...
      open(my $tr2h, ">$logfile")|| die " Can not create log $logfile";         # Are the bus transfers accounted for ? Mendatory
...
######printf $proh "\n";
      ...
      printf       " ... cecking reference         :: %s\n",   $opts_ref;
      printf       " ... cecking path to trace dir :: %s\n", ${$opts_ref}{report_dir};
      ...
      unless ( -d ${$opts_ref}{report_dir} ) {
            ...
            #exit; # No more exit of path not existing
      }

      if (  defined ${$opts_ref}{trace_file}   ) {
            my $file = ${$opts_ref}{trace_file};
            printf       " ... list trace file           :: %s \n", ${$opts_ref}{trace_file};
...
sub   analyze_tracefiles {
      ...
      foreach my $analyse ( @{$trace_ref} ){
         ...
         #printf "%*s%s\n"    , 15,'',                      ${$analyse}{test_case}  ;   # Quiet terminal output
         ...
         #printf "%*s%s\n"    , 15,'',                      ${$analyse}{report}     ;
          ...
          ${$opts_ref}{transferlist}= ${$analyse}{report_path}.'/'.${$opts_ref}{transferlist_default};
          ...
          ${$opts_ref}{transaction2}= ${$analyse}{report_path}.'/'.${$opts_ref}{transaction2_default};
...
sub   coverage_analysis {
      ...
      foreach my $analyse ( @{$trace_ref} ){
         ...
         #printf "%*s%s\n"    , 15,'',                      ${$analyse}{report}     ;
          ...
          ${$opts_ref}{transaction} = ${$analyse}{report_path}.'/'.${$opts_ref}{transaction_default};
          ${$opts_ref}{transaction2}= ${$analyse}{report_path}.'/'.${$opts_ref}{transaction2_default};
          ${$opts_ref}{address}     = ${$analyse}{report_path}.'/'.${$opts_ref}{address_default};
          ${$opts_ref}{report}      = ${$analyse}{report};
...
sub   read_transfer_list {
     ...
     #my $ptr_cur_cmd               =   ${${$ptr_ref}{adr}{first}};                 # Old hack to global pointer
      my $ptr_cur_cmd               =     ${$ptr_ref}{adr}{first};

     #my $buffer_stop               =   ${${$ptr_ref}{adr}{first}};                 # Old hack Interate through the whole list, stop at the top
...
sub   address_coverage_analysis {
      my    (   $trace_info                                                         # Input     Trace Header information
            ,   $ptr_ref                                                            #           Pointer into AHB transfer list
            ,   $opts_ref                                                           #           Command line options
            ,   $adr_ref                );                                          # Output    Address Coverage Analysis 
      my (  $trace_header_ref                                                       # Input     Trace   information from trace file header
...
sub   addr_cov {
      ...
      analyze_coverage            (   $opts_ref                 # Input     Command Line Options
