...
# Phone     408 807 6915
# 
# Purpose   Adding Monitors to the XFLOW dv testbench  
...
use strict;
use warnings;
use version; my $VERSION = version->declare('v1.03.03');                            # v-string using Perl API
...
#use lib							qw  (   ~/ws/perl/lib  );                           # Relative path UserHome to custom lib
use lib							qw  ( /mnt/ussjf-home/lfilor/ws/perl/lib );     # Add Include path to @INC
#use UTF8                    	qw  /   read_utf8                                   # 05/07/2019 change
...
use File::Header::Add           qw  /   add_header          /;                      # Add Header to file
use ARM::Monitor::Add           qw  /   add_new_monitor     /;                      # Add ARM Monitors
...
GetOptions  (   'input|in=s'            =>  \$opts{pseudo_seq},                     # Source    w/ read,write transaction definition
                ...
                'specification|spec=s'  =>  \$opts{spec},                           # Input     test specification
                                                                                    #           AXI Monitor specification
                'sequence|seq=s'        =>  \$opts{seq},                            # Output    test sequence

                'generate|gen=s'        =>  \$opts{output},                         #           axi_monitor_include.svi
...
elsif ( defined $opts{spec} )  {
    add_new_monitor( \%opts );                                                      # Add ARM Monitors
}
