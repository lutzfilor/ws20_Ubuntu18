...
our @EXPORT_OK  =   qw  (   get_references
                            get_attribute
                            get_accesspath
                            get_headers
                            get_section
...
sub     get_record  {
        ...
        if ((ref $dp_r) =~ m/HASH/ ) {
            foreach my $inst (keys %{$dp_r}) {
                ...
                ${$dp_r}{$inst}{data}{$zscope} = get_zdata  ($zscope,$zrecord);    # Return ref to zData
                #${$dp_r}{$inst}{data}{headers} = get_headers($zrecord,'headers',$LIMITx);
                ...
                ${$dp_r}{$inst}{data}{instances}        = $merged;
                ${$dp_r}{$inst}{data}{loctable}{raw}    = $loc_info;
                ${$dp_r}{$inst}{data}{rectable}{raw}    = $rec_info;
 
                log_msg('get_entry()','logs/log_headers');
                my $headers   = get_entry ($zrecord  ,'headers'  ,'list','logs/log_headers');
                ${$dp_r}{$inst}{data}{header} = $headers;      
                ...
                if ($hits1) {
                    ...
                    ${$dp_r}{$inst}{data}{loctable}{totalhits} = $hits11;
                }# if data included
                ...
                if ($hits2) {
                    ...
                    ${$dp_r}{$inst}{data}{loctable}{avgw}      = $hits21;
                }# if data included
                ...
                if ($hits3) {
                    ...
                    ${$dp_r}{$inst}{data}{rectable}{totalhits} = $hits31;
                }# if data included
                ...
                if ($hits4) {
                    ...
                    ${$dp_r}{$inst}{data}{rectable}{avgw}      = $hits41;
                }# if data included
                log_msg("\n",'logs/log_hits');
                ...
                #${$dp_r}{$inst}{data}{totalhit}= get_vpair ($totalhit,'val',',');
                #${$dp_r}{$inst}{data}{averagew}= get_vpair ($averagew,'val',',');
                #${$dp_r}{$inst}{data}{Toggles} = get_vpair ($data,'val',',');
