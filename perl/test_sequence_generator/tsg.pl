...
#           1.01.10     12/09/2018      Add LINT feature, integrating into System Verilog Simulation
#           1.01.11     12/17/2018      Decompose XML, Parsing XML
#                       12/18/2018      Fix XML-TAG to XML-TAG:TAG
#           1.01.12     12/19/2018      Add LINT log feature, for filter
#                                       Add LINT format features, crunch, left-align, right-align
#           1.01.13     12/20/2018      Document scripts
#                                       Clean up experimental
#           1.01.14     01/02/2019      Preserve ^M in XML element, retrieve XML prolog, string
#           1.01.15     01/04/2019      Parsing XML elements of XML string
#                                       Detecting, XML element, nested XML element open,
#                                       nested XML element close, self terminating XML element
#           1.01.16     01/07/2019      Extracting XML information
#                                       Virtual XML data structure
# ================================================================================================
...
#use Spreadsheet::ParseExcel;                                                       # available
...
Readonly my $XMLAN          =>      qr  {   \b\w+                   }xms;           # XML Attribute name
...
if  (      defined $opts{flush}) { flush_test_sequence  (   \%opts                  # Input     Command line Options
...
} elsif (  defined $opts{lint} ) { lint_test_sequence   (   \%opts  );              # Lint      Remove comment lines, enforce homogenity, increase robustness
} elsif (  defined $opts{seek} ) { seek_file_pattern    (   \%opts  );
} elsif (  defined $opts{open} ) { open_binary_files    (   );                      # Experimental code
} elsif (  defined $opts{map}  ) { map_subdirectory     (   );                      #           Extract subdirectory structures
} elsif (  defined $opts{maps} ) { map_subroutines      (   );                      #           Extract subroutine implementations
} elsif (  defined $opts{help} ) { online_help          (   \%opts  );              # Online    help
} else                           { user_help();                         }           # User      help
exit 0; # Main Exit
#=================================================================================================================================
# End of main()
#
...
sub   extract_format {
      ...
      foreach my $width ( @{${$opts_ref}{format}{$file_name}} ) {
        printf "%*s%s[%2s] : %s\n", $i,$p,'Column',$ci, ${${$opts_ref}{format}{$file_name}}[$ci] if( debug($n));
        $ci++;
      }#
      printf "\n\n" if( debug($n));
}#sub extract_format
...
sub   read_xml_file {
      ...
      for my $line ( @xml) {
            ...
            printf "%*s%*s: %s\n",$i,$p,4,$lc,$line;                                # print XML array line
...
sub   read_rels_file {
      ...
      for my $line ( @xml) {
          ...
          chomp $line;
          printf "%*s%*s: %s",$i,$p,4,$lc,$line;
          push ( @{${$opts_ref}{tmp}}, sprintf "%*s%*s: %s",$i,$p,4,$lc,$line);
      }#for
      printf "\n\n";                                                                # spacer
      $lc = 0;
      for my $line ( @xml) {
...
sub   print_xml_string  {
      ...
      parse_xmle( \@xmle,'tmp4');                                                   # Experimental
      return @xmle;
}#sub print_xml_string
...
sub   parse_xmle {
      ...
      #(<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="872" uniqueCount="268">)
      #(</si>)
      #(</sst>)

      #Readonly my $TAGN         =>      qr  {   \B(w+)\b                }xms;               # XML Tag name
      #Readonly my $ANAME        =>      qr  {   \b\w+                   }xms;               # XML Attribute name
      #Readonly my $AVALUE       =>      qr  {   ['].*[']|["].*["]       }xms;               # XML Attribute value single or double quoted string

      #Readonly my $XMLA         =>      qr  {   $ANAME\B[=]\B$AVALUE    }xms;               # XML Attribute
      #Readonly my $XML_ELEMENT  =>      qr  {   [^><]+                  }xms;               # Any character Inside opening and closing XML TAG

      #Readonly my $XML_NEO      =>      qr  {   [<]$TAGN\s+$XMLA*\s*[>] }xms;               # XML nested element opening tag w/ optional attribute
      #Readonly my $XML_NEC      =>      qr  {   [<][/](.*)[>]           }xms;               # XML nested element closing tag
      #Readonly my $XMLE_ST      =>      qr  {   [<]$TAGN\s+$XMLA*\s*[/][>]  }xms;           # XML self terminating elememt - without closing tag
      #Readonly my $XMLE         =>      qr  {   [<]$TAGN\s+$XMLA*\s*[>]                     # XML opening tag w/ optional attribute
      #                                          $XML_ELEMENT                                # XML element data
      #                                          [<][/]$TAGN [>]         }xms;               # XML closing tag matching
      ## if ( $xmle =~   m{^\s*(<(?|/|\?)?)                                        # XML Start TAG begin
      ##                   #   ([_a-zA-Z:]+)                                       # Forgot numbers and colon
      ...
      push( @{${$opts_ref}{$tmp}}, '' );
      for my $xmle ( @{$xmle_ref} ) {
         if      ( $xmle =~ m{  $XML_PRO    }xms  )  {
                $msg = sprintf "%*s%s :: %s",$i+$n*4,$p,$xmle,'XML Prolog';
                push( @{${$opts_ref}{$tmp}}, $msg );
         } elsif ( $xmle =~ m{  $XML_NEC    }xms )  {
                $msg = sprintf "%*s%s :: %s",$i+$n*4,$p,$xmle,'Nested XML element closing tag';
                push( @{${$opts_ref}{$tmp}}, $msg );
         } elsif ( $xmle =~ m{  $XMLE_ST    }xms )  {
                $msg = sprintf "%*s%s :: %s",$i+$n*4,$p,$xmle,'Self terminated XML element';
                push( @{${$opts_ref}{$tmp}}, $msg );
         } elsif ( $xmle =~ m{  $XMLE       }xms )  {
                $msg = sprintf "%*s%s :: %s",$i+$n*4,$p,$xmle,'Regular XML element';
                push( @{${$opts_ref}{$tmp}}, $msg );
         } elsif ( $xmle =~ m{  $XML_NEO    }xms )  {
                $msg = sprintf "%*s%s :: %s",$i+$n*4,$p,$xmle,'Nested XML element opening tag';
                push( @{${$opts_ref}{$tmp}}, $msg );
         } else {
                $msg = sprintf "%*s%s :: %s",$i+$n*4,$p,$xmle,'XMLE not covered yet';
                push( @{${$opts_ref}{$tmp}}, $msg );
                if  ( $xmle =~ m{   ^[<] 
                                    ($TAGN)
                                    \s+.* 
                    ...
                    push( @{${$opts_ref}{$tmp}},sprintf "%*s%s :: %s",$i+$n*4,$p,'XML TAG name',$1);
                } else {
                    push( @{${$opts_ref}{$tmp}},sprintf "%*s%s :: %s",$i+$n*4,$p,'XML TAG name','NOT detected');
                }
         }#else
      }#for
}#sub parse_xmle
...
sub   log_xml_parsing  {
      ...
      my $n = subroutine('name');
      printf "%*s%s()\n",$i,$p,$n if( debug($n));
      ${$opts_ref}{xml_log2} //= 'tmp_parsing.log';
      printf "%*s%s : %s\n", $i,$p, 'TestSequenceName', ${$opts_ref}{xml_log2};
      open(my $xmlh,'>:encoding(UTF-8)',${$opts_ref}{xml_log2});
      foreach my $line ( @{${$opts_ref}{tmp2}} ) {
         printf $xmlh "%s\n",$line;
      }#
      close(  $xmlh );
}#sub log_xml_decoding
